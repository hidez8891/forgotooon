    ...
    return (
        <>
            {todos.map((todo) =>
                <View key={todo.id}>
                    <WrapItem item={todo} />
                    <Button onPress={() => remove(todo.id)} title="" data-test="del-btn" />
                    <Button onPress={() => update(todo.id, !todo.done)} title="" data-test="up-btn" />
                </View>
            )}
...
describe("Todo Store", () => {
    ...
    it("can add items", () => {
        ...
        for (let i = 0; i < 3; ++i) {
            renderer.act(() => {
                const button = component.root.findByProps({ "data-test": "add-btn" });
    ...
    it("can remove items", () => {
        ...
        renderer.act(() => {
            const button = component.root.findAllByProps({ "data-test": "del-btn" })[0];
            button.props.onPress();
        });
    ...
    it("can update flag", () => {
        ...
        renderer.act(() => {
            const button = component.root.findAllByProps({ "data-test": "up-btn" })[0];
            button.props.onPress();
        });
